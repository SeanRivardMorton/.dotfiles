...
async function getApiKeyFromSecretsManager(): Promise<string> {
  ...
  try {
    const secretName = config.awsSecretName;
...
// Initialize client with environment-specific URLs - API key will be set later
const baseURL =
  process.env.CUSTOMER_CARE_API_BASE_URL || envConfig.customerCareApiBaseUrl;
const customerBaseURL =
  process.env.CUSTOMER_API_BASE_URL || envConfig.customerApiBaseUrl;
const accountsBaseURL =
  process.env.ACCOUNTS_API_BASE_URL || envConfig.accountsApiBaseUrl;
const cardsBaseURL =
  process.env.CARDS_API_BASE_URL || envConfig.cardsApiBaseUrl;
const transactionsBaseURL =
  process.env.TRANSACTIONS_API_BASE_URL || envConfig.transactionsApiBaseUrl;
const onboardingBaseURL =
  process.env.ONBOARDING_API_BASE_URL || envConfig.onboardingApiBaseUrl;
const databricksBaseURL = process.env.DATABRICKS_BASE_URL || "";
...
const DatabricksExecuteQuerySchema = z.object({
  warehouse_id: z
    .string()
    .describe("The ID of the SQL warehouse to execute the query on"),
  statement: z.string().describe("The SQL statement to execute"),
  catalog: z.string().optional().describe("The catalog to use for the query"),
  schema: z.string().optional().describe("The schema to use for the query"),
  parameters: z
    .record(z.any())
    .optional()
...
const DatabricksQueryStatusSchema = z.object({
  statement_id: z.string().describe("The statement ID to check status for"),
});

const DatabricksQueryResultsSchema = z.object({
  statement_id: z.string().describe("The statement ID to get results for"),
  chunk_index: z
    .number()
    .optional()
    .describe("The chunk index to retrieve (default: 0)"),
});

const DatabricksCancelQuerySchema = z.object({
  statement_id: z.string().describe("The statement ID to cancel"),
});
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
        ...
        inputSchema: {
          ...
          properties: {
            warehouse_id: {
              type: "string",
              description:
                "The ID of the SQL warehouse to execute the query on",
            },
            statement: {
              type: "string",
      ...
      {
        name: "databricks_query_status",
        description: "Get the status of a Databricks query by statement ID",
        inputSchema: {
          type: "object",
          properties: {
            statement_id: {
              type: "string",
              description: "The statement ID to check status for",
            },
          },
      ...
      {
        name: "databricks_query_results",
        description: "Get the results of a completed Databricks query",
        inputSchema: {
          type: "object",
          properties: {
            statement_id: {
              type: "string",
              description: "The statement ID to get results for",
            },
            chunk_index: {
              type: "number",
              description: "The chunk index to retrieve (default: 0)",
            },
          },
          required: ["statement_id"],
        },
      ...
      {
        name: "databricks_cancel_query",
        description: "Cancel a running Databricks query",
        inputSchema: {
          type: "object",
          properties: {
            statement_id: {
      ...
      {
        name: "databricks_execute_and_wait",
        description:
          "Execute a SQL query on Databricks and wait for results (convenience method)",
        inputSchema: {
          type: "object",
          properties: {
            warehouse_id: {
            ...
            max_wait_time: {
              type: "number",
              description:
                "Maximum time to wait for query completion in milliseconds (default: 30000)",
            },
          },
          required: ["warehouse_id", "statement"],
        },
  ...
  try {
    switch (name) {
      ...
      case "databricks_execute_query": {
        const { warehouse_id, statement, catalog, schema, parameters } =
          DatabricksExecuteQuerySchema.parse(args);
        const result = await client.executeDatabricksQuery(
          warehouse_id,
          statement,
          catalog,
          schema,
      ...
      case "databricks_query_status": {
        const { statement_id } = DatabricksQueryStatusSchema.parse(args);
        const result = await client.getDatabricksQueryStatus(statement_id);
        return {
          content: [
            {
              type: "text",
      ...
      case "databricks_query_results": {
        const { statement_id, chunk_index } =
          DatabricksQueryResultsSchema.parse(args);
        const result = await client.getDatabricksQueryResults(
          statement_id,
          chunk_index || 0,
        );
        return {
      ...
      case "databricks_cancel_query": {
        const { statement_id } = DatabricksCancelQuerySchema.parse(args);
        const result = await client.cancelDatabricksQuery(statement_id);
        return {
          content: [
            {
              type: "text",
      ...
      case "databricks_execute_and_wait": {
        const {
          warehouse_id,
          statement,
          catalog,
          ...
          parameters,
          max_wait_time,
        } = DatabricksExecuteQuerySchema.extend({
          max_wait_time: z.number().optional(),
        }).parse(args);
        const result = await client.executeDatabricksQueryAndWait(
          warehouse_id,
          statement,
          catalog,
          schema,
...
async function main() {
  try {
    ...
    const databricksAuthType = process.env.DATABRICKS_AUTH_TYPE || "token";

    if (databricksAuthType === "token") {
      const databricksToken = process.env.DATABRICKS_TOKEN;
      if (databricksToken) {
        client.setDatabricksToken(databricksToken);
        console.error("Databricks token successfully set");
      } else {
        console.error(
          "Warning: DATABRICKS_TOKEN not provided. Databricks functionality will be unavailable.",
        );
      }
    } else if (databricksAuthType === "service-principal") {
      const clientId = process.env.DATABRICKS_CLIENT_ID;
      const clientSecret = process.env.DATABRICKS_CLIENT_SECRET;
      const tenantId =
        process.env.DATABRICKS_TENANT_ID || process.env.AZURE_TENANT_ID;

      if (clientId && clientSecret && tenantId) {
        try {
          await client.setDatabricksAuth({
          ...
          console.error(
            "Databricks service principal authentication successfully configured",
          );
        } catch (error) {
          console.error(
            "Failed to configure Databricks service principal authentication:",
            error,
          );
          console.error("Databricks functionality will be unavailable.");
        }
      } else {
        console.error(
          "Warning: Service principal credentials not fully configured. Databricks functionality will be unavailable.",
        );
        console.error(
          "Required: DATABRICKS_CLIENT_ID, DATABRICKS_CLIENT_SECRET, and DATABRICKS_TENANT_ID",
        );
      }
    }
    ...
    console.error(
      `Starting Customer Care MCP Server in ${currentEnv.toUpperCase()} environment`,
    );
    console.error(`Using API endpoints for ${currentEnv}:`);
    console.error(`  - Customer Care: ${baseURL}`);
    console.error(`  - Customers: ${customerBaseURL}`);
    console.error(`  - Accounts: ${accountsBaseURL}`);
    console.error(`  - Cards: ${cardsBaseURL}`);
    console.error(`  - Transactions: ${transactionsBaseURL}`);
    console.error(`  - Onboarding: ${onboardingBaseURL}`);
